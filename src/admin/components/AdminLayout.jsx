...
export default function AdminLayout({ children }) {
  ...
  return (
    <div className="h-screen flex bg-[#f0f4f8] overflow-hidden">
      {/* Mobile overlay */}
      {sidebarOpen && (
      ...
      {/* Sidebar */}
      <aside
        className={`fixed top-0 left-0 h-full w-64 z-40 flex flex-col transition-transform duration-300 lg:translate-x-0 lg:sticky lg:top-0 lg:h-screen lg:z-auto ${
        ...
        <div className="px-4 py-4 border-t border-white/10">
          <div className="flex items-center gap-3 px-3 py-2 mb-2">
            <div className="w-8 h-8 rounded-full bg-[#ED6D23]/20 border border-[#ED6D23]/40 flex items-center justify-center shrink-0">
              <span className="text-[#ED6D23] text-xs font-bold uppercase">{admin?.username?.[0] || 'A'}</span>
            </div>
