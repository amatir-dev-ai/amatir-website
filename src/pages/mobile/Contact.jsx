...
export default function ContactEnquirePage() {
  ...
  const getFieldStyle = (key) => ({
    borderStyle: "solid",
    borderColor: focus[key] ? COLORS.blue : COLORS.ink,
    borderWidth: "0.08rem",
    padding: "0.8rem 1rem",
    fontSize: "1rem",
    borderRadius: "0.5rem",
    boxShadow: focus[key] ? "0 0 0 2px rgba(28,54,100,0.12)" : "none",
    textAlign: "left",
  });
  ...
  return (
    <main id="mobile-contact" className="w-screen py-12 md:py-16 overflow-x-hidden bg-white text-black" style={{ marginTop: "60px" }}>
      ...
      <section className="w-screen px-[4vw]">
        <div className="flex flex-col items-stretch md:flex-row" style={{ gap: "5vw" }}>
          {/* CONTACT (left on desktop, second on mobile) */}
          <div className="order-2 grow w-full md:order-1 md:basis-[48%]">
            <h1
          ...
          <div className="order-1 grow w-full md:order-2 md:basis-[48%]">
            <h2
              className="cent-schbk-cyrill font-semibold leading-tight"
              style={{
...
function Field({ label, children, className = "", style = {} }) {
  return (
    <label className={`block ${className}`} style={{ marginBottom: "1rem", ...style }}>
      <span
