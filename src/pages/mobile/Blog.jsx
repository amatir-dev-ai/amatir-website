...
const BLOGS = {
  blog1: {
    ...
    read: "7 min read",
    hero:
      "/images/Rectangle_99_xj6xpx.jpg",
    inlineImg:
      "/images/Blog_2nd_pic_flegar.jpg",
    heroAlt:
  ...
  blog4: {
    ...
    img: "/images/CBSE_in_Gurukuls__How_It_Works_u4yzti.jpg",
    date: "25 Nov 2025",
    read: "5 min read",
  },
  blog5: {
...
function CardsCarousel({ posts }) {
  ...
  const prev = () => setI((p) => (p - 1 + posts.length) % posts.length);
  const next = () => setI((p) => (p + 1) % posts.length);
  ...
  return (
    <main className="w-full bg-white text-[#1C3664]">
      ...
      <section className="mx-auto w-[92%] max-w-[980px] pt-12 md:pt-16">
        <h1 className="text-center font-serif text-[28px] leading-tight md:text-[36px] mt-12">
          {post.title}
        </h1>
      ...
      <article className="mx-auto w-[92%] max-w-[820px] py-10 md:py-14">
        ...
        <figure className="mt-6 overflow-hidden rounded-[10px] border border-slate-200/80">
