...
export default function ContactEnquirePage() {
  useSEO({
    ...
    description:
      'Contact Amatir Kanya Gurukul in Kurukshetra, Haryana for admissions and enquiries. Visit our campus, call our team or connect online for more information.',
    keywords: '',
  });
  ...
  useEffect(() => {
    ...
    if (container) {
      ...
      const baseUrl = 'https://crm.mediagarh.com/CRM/forms/public/33';
    ...
    <main className="w-screen py-8 overflow-x-hidden bg-white text-black" style={{ marginTop: '60px' }}>
      ...
      <section className="w-screen px-[4vw]">
        <div className="flex flex-col items-start md:flex-row" style={{ gap: '5vw' }}>
          ...
          <div className="order-1 grow md:order-2 md:basis-[48%]">
            ...
            >
              ...
              <div ref={crmContainerRef} id="crm-form-container-33" className="w-full h-full" />
            </div>
          </div>
        </div>
        ...
        <div
          ...
          style={{
            height: '0.15rem',
            background: 'linear-gradient(90deg, rgba(28,54,100,0.12), rgba(237,109,35,0.28), rgba(28,54,100,0.12))',
            marginTop: '8vh',
          }}
        />
      </section>
...
function ContactRow({ iconSrc, label, href, children }) {
  const content = (
    <span className="univers-regular" style={{ fontSize: 'clamp(1rem, 1.8vw, 1.15rem)', lineHeight: 1.6 }}>
      {children}
    </span>
  );
  return (
