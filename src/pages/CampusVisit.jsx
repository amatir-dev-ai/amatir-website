...
const ASSETS = {
  ...
  accent: "/images/Orange_Icon_nyaf8h.svg",
...
export default function CampusVisit() {
  ...
  // Dynamically load the CRM iframe on component mount
  useEffect(() => {
  ...
  return (
    <div className="min-h-screen w-full bg-white text-slate-800 overflow-x-hidden">
      ...
      >
        ...
        />
        <div className="absolute inset-0" style={{ background: "rgba(14,35,73,0.35)" }} />
      ...
      {/* ====== SEE IT FOR YOURSELF ====== */}
      <section className={`mx-auto ${isSmallMobile ? 'w-[95%]' :
        isNarrowScreen ? 'w-[94%]' :
          'w-[92%]'
        } ${isNarrowScreen ? 'max-w-[1200px]' : 'max-w-[1180px]'} ${isSmallMobile ? 'py-8' :
          isMobile ? 'py-10' :
            isLandscapeTablet ? 'py-12' :
              'py-12 md:py-20'
        }`}>
        <div className={`flex flex-col items-start ${isMobile ? 'gap-6' :
          isLandscapeTablet ? 'gap-8' :
            'gap-10'
          } ${isTablet || isLandscapeTablet ? 'md:flex-row md:items-center md:gap-8' :
            'md:flex-row md:items-center md:gap-14'
          }`}>
          <div className="w-full md:w-1/2">
            <h2 className={`mb-4 font-serif ${isSmallMobile ? 'text-[24px]' :
              isMobile ? 'text-[28px]' :
                isLandscapeTablet ? 'text-[32px]' :
                  'text-[30px] md:text-[40px]'
              }`} style={{ color: NAVY2 }}>
              See It for Yourself
            </h2>
            <div className={`space-y-5 text-slate-700 ${isSmallMobile ? 'text-[14px] leading-6' :
              isMobile ? 'text-[15px] leading-7' :
                isLandscapeTablet ? 'text-[16px] leading-7' :
                  'text-[16px] leading-7 md:text-[17px] md:leading-8'
              }`}>
              <p>
                Step into the world of Amatir—an inspiring girls gurukul in Kurukshetra, Haryana. Nestled in a scenic, secure, and pollution-free 30-acre campus, Amatir offers a vibrant residential experience for over 300 girls. During your interactive school visit, you’ll explore our thoughtfully designed school and hostel facilities, meet our educators, and witness a unique blend of modern CBSE education and timeless Indian values.
              </p>
              <p>
                We place special emphasis on discipline, culture, and holistic development—ensuring that every child grows with clarity, confidence, and character.
              </p>
              <p>
                We place special emphasis on discipline, culture, and holistic development—ensuring that every child grows with clarity, confidence, and character.
              </p>
          ...
          <div className="w-full md:w-1/2">
            <div className="relative overflow-hidden rounded-xl shadow-lg">
              ...
              <button
                ...
                className={`absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 rounded-full bg-white/90 backdrop-blur hover:bg-white ${isSmallMobile ? 'p-3' :
                  isMobile ? 'p-4' :
                    isLandscapeTablet ? 'p-4' :
                      'p-5'
                  }`}

                onClick={() => setShowVideo(true)}
              >
                <Play className={`${isSmallMobile ? 'h-6 w-6' :
                  isMobile ? 'h-7 w-7' :
                    isLandscapeTablet ? 'h-7 w-7' :
                      'h-8 w-8'
                  }`} style={{ color: NAVY }} />
              </button>
            </div>
              ...
              >
                ...
                <div className="relative z-10 max-w-4xl mx-auto px-4 md:px-6 pt-18">
                  <button
                    ...
                    className="inline-flex items-center bg-white text-slate-800 px-4 py-2 rounded-lg cursor-pointer"
                  >
                    Close
                  </button>
      ...
      <section
        className={`${isSmallMobile ? 'py-12' :
          isMobile ? 'py-14' :
            isLandscapeTablet ? 'py-16' :
              'py-16 md:py-20'
          }`}
        style={{ background: LIGHT_BG }}
      >
        <div
          className={`mx-auto ${isSmallMobile ? 'w-[95%]' :
            isNarrowScreen ? 'w-[94%]' :
              'w-[92%]'
            } ${isNarrowScreen ? 'max-w-[800px]' : 'max-w-[760px]'}`}
        >
          <h3
            className={`text-center font-serif leading-tight ${isSmallMobile ? 'text-[24px]' :
              isMobile ? 'text-[26px]' :
                isLandscapeTablet ? 'text-[30px]' :
          ...
          <p
            className={`mx-auto mt-3 ${isNarrowScreen ? 'max-w-[720px]' : 'max-w-[680px]'
              } text-center ${isSmallMobile ? 'text-[14px] leading-6' :
                isMobile ? 'text-[15px] leading-7' :
                  isLandscapeTablet ? 'text-[15px] leading-7' :
                    'text-[15px] md:text-[16px] leading-7'
              } text-slate-600`}
          >
            Please fill out the form below to schedule your visit and let us know your preferred
            date and time. Our admissions team will get in touch with you shortly.
          </p>
      ...
      <section className="relative w-full">
        <img
          ...
          alt=""
          className={`w-full object-cover ${isSmallMobile ? 'h-[200px]' :
            isMobile ? 'h-[240px]' :
              isLandscapeTablet ? 'h-[280px]' :
                'h-[260px] md:h-[300px]'
            }`}
        />
        <div className="absolute inset-0" style={{ background: "rgba(237,109,35,0.88)" }} />
        <div className="absolute inset-0 flex flex-col items-center justify-center px-4 text-center text-white">
          <h3 className={`font-serif ${isSmallMobile ? 'text-[22px]' :
            isMobile ? 'text-[24px]' :
              isLandscapeTablet ? 'text-[30px]' :
                'text-[26px] md:text-[34px]'
            }`}>Unlock her future!</h3>
          <p className={`mt-2 ${isNarrowScreen ? 'max-w-[720px]' : 'max-w-[680px]'
            } ${isSmallMobile ? 'text-[14px] leading-6' :
              isMobile ? 'text-[15px] leading-7' :
                isLandscapeTablet ? 'text-[15px] leading-7' :
      ...
      <section className="relative w-full">
        <iframe
          ...
          className={`w-full ${isSmallMobile ? 'h-[300px]' :
            isMobile ? 'h-[360px]' :
              isLandscapeTablet ? 'h-[400px]' :
                'h-[420px] md:h-[520px]'
            }`}
