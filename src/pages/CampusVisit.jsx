...
export default function CampusVisit() {
  ...
  return (
    <div className="min-h-screen w-full bg-white text-slate-800 overflow-x-hidden">
      ...
      >
        <img src={ASSETS.hero} alt="Campus aerial" className="absolute inset-0 h-full w-full object-cover" />
        ...
        <div className="relative h-full z-10 mx-auto max-w-[1262px] px-4 flex items-end mt-[-8%] justify-center">
          ...
          >
            <div className="flex justify-center mb-4">
              <img alt="Amatir Icon" className="w-[60px] h-auto" src={ASSETS.accent} />
            </div>
            <h1
      ...
      <section
        className={`mx-auto ${
          isSmallMobile ? 'w-[95%]' : isNarrowScreen ? 'w-[94%]' : 'w-[92%]'
        } ${isNarrowScreen ? 'max-w-[1200px]' : 'max-w-[1180px]'} ${
          isSmallMobile ? 'py-8' : isMobile ? 'py-10' : isLandscapeTablet ? 'py-12' : 'py-12 md:py-20'
        }`}
      >
        <div
        ...
        >
          <div className="w-full md:w-1/2">
            ...
            >
              <p>
                Step into the world of Amatir—an inspiring girls gurukul in Kurukshetra, Haryana. Nestled in a scenic,
                secure, and pollution-free 30-acre campus, Amatir offers a vibrant residential experience for over 300
                girls. During your interactive school visit, you’ll explore our thoughtfully designed school and hostel
                facilities, meet our educators, and witness a unique blend of modern CBSE education and timeless Indian
                values.
              </p>
              <p>
                We place special emphasis on discipline, culture, and holistic development—ensuring that every child
                grows with clarity, confidence, and character.
              </p>
              <p>
                We place special emphasis on discipline, culture, and holistic development—ensuring that every child
                grows with clarity, confidence, and character.
          ...
          <div className="w-full md:w-1/2">
            <div className="relative overflow-hidden rounded-xl shadow-lg">
              <img src={ASSETS.videoThumb} alt="Campus life video" className="h-full w-full object-cover" />
              <button
                aria-label="Play video"
                className={`absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 rounded-full bg-white/90 backdrop-blur hover:bg-white ${
                  isSmallMobile ? 'p-3' : isMobile ? 'p-4' : isLandscapeTablet ? 'p-4' : 'p-5'
                }`}
                onClick={() => setShowVideo(true)}
              >
                <Play
                  className={`${
                    isSmallMobile ? 'h-6 w-6' : isMobile ? 'h-7 w-7' : isLandscapeTablet ? 'h-7 w-7' : 'h-8 w-8'
                  }`}
                  style={{ color: NAVY }}
                />
              </button>
      ...
      <section
        className={`${isSmallMobile ? 'py-12' : isMobile ? 'py-14' : isLandscapeTablet ? 'py-16' : 'py-16 md:py-20'}`}
      ...
      >
        ...
        >
          ...
          <p
            className={`mx-auto mt-3 ${isNarrowScreen ? 'max-w-[720px]' : 'max-w-[680px]'} text-center ${
              isSmallMobile
                ? 'text-[14px] leading-6'
                : isMobile
                  ? 'text-[15px] leading-7'
          ...
          >
            Please fill out the form below to schedule your visit and let us know your preferred date and time. Our
            admissions team will get in touch with you shortly.
          </p>
          ...
          >
            <div ref={crmContainerRef} id="crm-form-container-35" className="w-full h-full" />
          </div>
        </div>
      </section>
      ...
      <section className="relative w-full">
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center px-4 text-center text-white">
          ...
          <a
            href="/contact"
            className={`mt-5 inline-flex items-center gap-2 rounded-full px-5 py-2 font-semibold text-white ${
              isSmallMobile ? 'text-[14px]' : 'text-[15px]'
            }`}
